...
class RenameIssueStatusOrders < ActiveRecord::Migration
  def up
    ...
    rename_table :issue_status_orders, :agile_ranks if table_exists?(:issue_status_orders)
  ...
  def down
    ...
    rename_table :agile_ranks, :issue_status_orders if table_exists?(:agile_ranks)

    add_index :issue_status_orders, :issue_id if table_exists?(:issue_status_orders) unless index_exists?(:issue_status_orders, :issue_id)
    add_index :issue_status_orders, :position if table_exists?(:issue_status_orders) unless index_exists?(:issue_status_orders, :position)
  end
end

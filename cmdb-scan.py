...
#* * * * * /opt/lmu.ansible.playbooks/cmdb-scan.py --private-key=/home/fachadmin/.ssh/id_rsa
#* * * * * ansible -u ansible -m setup -i /opt/lmu.ansible.playbooks/inventory --tree /opt/lmu.ansible.playbooks/unchecked-out/ --private-key=/home/fachadmin/.ssh/id_rsa all
#*/5 * * * * ansible-cmdb -i /opt/lmu.ansible.playbooks/inventory /opt/lmu.ansible.playbooks/checked-out/ > /opt/lmu.ansible.playbooks/cmdb/index.html
...
def execute_ansible_setup(inventory='/opt/lmu.ansible.playbooks/inventory', scope='all', limit=[], user='ansible', key='/home/fachadmin/.ssh/id_rsa', out_directory='/opt/lmu.ansible.playbooks/unchecked-out/'):
	"""
	"""
...
def check_and_copy_ansible_setup_output(input_directory='/opt/lmu.ansible.playbooks/unchecked-out/', output_directory='/opt/lmu.ansible.playbooks/checked-out/'):
		...
		for server_file in listdir(input_directory):
			if isfile(os.path.join(input_directory, server_file)):
				with open(os.path.join(input_directory, server_file), 'r') as f:
					...
					if not data.has_key('unreachable'):
						shutil.copy2(os.path.join(input_directory, server_file), output_directory)


def execute_ansible_cmdb(inventory='/opt/lmu.ansible.playbooks/inventory',
